...
uses
	...
	Controls, vvsConsts, vvsThreadList, HTTPApp, Dialogs, vvsFileMgmt, vvSvcStatus;
	...
	try
		...
		on E: Exception do begin //colocar como registro de depuração, por se tratar de erro comum
			TLogFile.LogDebug(Format('Falha de comunicação com o servidor pai desta instância(%s) na porta(%d).'#13#10,
				[Self.tcpclnt.Host, Self.tcpclnt.Port]) + E.Message, DBGLEVEL_ALERT_ONLY);
			raise;
		end;
	end;
end;
...
begin
	...
	try
		...
		if (not TStrHnd.startsWith(sHeader, STR_BEGIN_SESSION_SIGNATURE)) then begin
			...
			TLogFile.LogDebug(Format('Falha de protocolo, cadeia recebida="%s"', [sHeader]), DBGLEVEL_ALERT_ONLY);
		...
		try
			...
			if (ret = STR_OK_PACK) then begin
				...
				TLogFile.Log(Format('Cliente("%s") desconectado abruptamente com o seguinte retorno("%s")', [sClientName, ret]),
					lmtWarning);
			end;
		end;
	end;
...
begin
	...
	try
		...
		//Self.FSessionFileTransfer.Handle ?== downID

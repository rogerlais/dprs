...
var
	LogFileName: string;
begin
	LogFileName := TFileHnd.ConcatPath([GlobalConfig.PathServiceLog, TBioFilesService.LogFilePrefix() + FormatDateTime('YYYYMMDD',
		Now())]) + '.log';
	try
		...
		try
			...
			on E: Exception do begin
				...
				TLogFile.LogDebug(Format('Sem conectividade com a intranet(%s): %s'#13#10'Tentativa(%d)',
					[GlobalConfig.ServerName, E.Message, x + 1]), DBGLEVEL_ULTIMATE);
				Sleep(1000);
			end;
		end;
...
begin
	...
	Self.mailMsgNotify.Body.Text := NotificationText + #13#10'****** Arquivo de configuração ******' + #13#10 +
		GlobalConfig.ToString; //insere arquivo de configuração ao final
	try

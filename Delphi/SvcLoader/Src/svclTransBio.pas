...
uses
	SysUtils, Windows, Classes, XPFileEnumerator, XPThreads, svclTCPTransfer, System.Generics.Defaults;
...
type
	...
	TTesteSingle = class(TSingletonImplementation)
	public
		constructor createChildEntity();
	end;
...
begin
	...
	if (not SameText(GlobalConfig.TransbioConfig.Elo2TransBio, GlobalConfig.TransbioConfig.PathBio)) then begin
		BioFilesService.SendMailNotification
			...
			+ 'ELO=' + GlobalConfig.TransbioConfig.Elo2TransBio + #13#10 + 'TransBio=' + GlobalConfig.TransbioConfig.PathBio);
	end;
...
begin
	...
	if (not MoveFile(PChar(SrcFile.FullName), PChar(DestPath + '\' + SrcFile.Name))) then begin
		...
		if (not MoveFile(PChar(SrcFile.FullName), PChar(DestPath))) then begin
			TLogFile.Log('Falha movendo arquivo transmitido para o repositório definitivo do computador primário'#13#10 +
...
constructor TTesteSingle.createChildEntity;
begin
	inherited Create;
end;

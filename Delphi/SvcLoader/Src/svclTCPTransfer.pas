...
begin
	...
	try
		...
		on E: Exception do begin //colocar como registro de depuração, por se tratar de erro comum
			TLogFile.LogDebug('Falha de comunicação com o servidor de recebimento de arquivos'#13#10 + E.Message,
				DBGLEVEL_ALERT_ONLY);
			raise;
		end;
	end;
...
begin
	...
	try
		...
		repeat
			...
			TLogFile.LogDebug(Format('Recebida cadeia do cliente(%s) ao servidor:'#13#10'arquivo="%s"'#13#10'criação=%s'#13#10 +
				'acesso=%s'#13#10'Modificação=%s'#13#10'tamanho=%s'#13#10'hash=%s'#13#10, [retClientName, sfilename, smodifiedDate,
...
begin
	...
	if (GlobalConfig.RunAsServer) then begin
		Self.TrayIcon.Hint := Format('SESOP - Replicação de arquivos de biometria' + #13#10 + rtVersion + #13#10 +
			'Arquivos recebidos = %d' + #13#10, [Self.FSessionFileCount]);
	end else begin
		Self.TrayIcon.Hint := Format('SESOP - Replicação de arquivos de biometria' + #13#10 + rtVersion + #13#10 +
			'Arquivos enviados na sessão = %d' + #13#10, [Self.FSessionFileCount]);
